...
CREATE INDEX IF NOT EXISTS idx_rules_priority ON category_rules(priority, active);
...
CREATE INDEX IF NOT EXISTS idx_tx_needs_cat  ON transactions(category_id) WHERE category_id IS NULL AND is_transfer = 0;
...
-- Transfer Plans ---------------------------------------------------------
-- Each row is one item that gets transferred at the start of the month.
-- Items are grouped by to_account_id to show totals per destination.
